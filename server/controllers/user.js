      ...
      if (!userData) {
        ...
        if (req.files) {
          ...
          if (fileExt !== "png" && fileExt !== "jpg") {
            res.status(400).send({
              message:
                "La extensión no es válida. (Extensiones permitidas: .png y .jpg)",
            });
          } else {
            ...
            ); //Tomamos parámetro y se lo pegamos a la variable // Esto devuelve un error o un userUpdate
          }
        }
      }
    }
...
function getAvatar(req, res) {
  ...
  fs.stat(filePath, (err) => {
    if (err) {
      res.status(404).send({
        message: /* ${err}, */'El avatar que buscas no existe',
      });
    } else {
...
function signUpAdmin(req, res) {
  ...
  if(!password) {
    res.status(500).send({
      message: 'La contraseña es obligtoria'
    });
  } else {
